...
This first section incorporates a number of the common questions from students and the given answers.
...
This blog is one that we quite like: [How to write paragraphs](https://medium.com/advice-and-help-in-authoring-a-phd-or-non-fiction/how-to-write-paragraphs-80781e2f3054/){target="_blank"} (scroll down a little until you see the "how to write paragraphs heading). We do not agree with everything it says, in particular point 5 that a paragraph can't be more than 250 words, but a lot of the other points to watch out for are really helpful, and the general overall principle of the structure of a paragraph is helpful as well - you may also know it as the Point, Evidence, Explain, Link approach we looked at earlier in the semester.

<!-- This is an additional blog on paragraphs that gives a similar idea and is recommended by Gaby: https://libguides.hull.ac.uk/grammar/paragraphs -->
...
* Something like "The relationship between X and Y" or "The effect of X on Y" is fine at this stage depending on what analysis you did.
* That said a title that states the main findings can be really effective "Postgraduates show greater self-reported test anxiety than undergraduates."
* Titles should have enough info to know what the report is about. Make it clear and understandable; not complicated; avoid jargon
...
*	Titles that state No effect (or no relationship) found between ... are acceptable
...
* Is included in the word count and should be roughly 250 words.
...
* Try to avoid ending on "The limitations of our study are discussed in great length" or words to that effect. Remember the abstract is the first thing people read after the title so you don't want to destroy your paper before someone even reads the Introduction.
...
*	Starts on a new page after the abstract - i.e. take a new page after the abstract and start the introduction at the top of that new page.
*	No need to rewrite the title at top of introduction as it is on the title page.
* Can include a heading of "Introduction" centred and in bold if it helps but not required in APA formatting and introduction can start on first line of page.
...
*	How broad should you be at the start? "Psychology is interesting..." is too broad. Starting general on how we understand learning is fine but can be even more focused and start on the importance of one of your variables - e.g. Self-efficacy is the.....
...
* It can help to structure the introduction around one of your variables and make that the key focus of the paper. Key variable could be, for example, either one of the continuous variables of the correlation or the categorical variable of the t-test.
*	Possible five paragraph structure - introduction on general area and key variable, more focused info on key variable, info on second variable, theory underlying ideas, current study paragraph (including research questions and hypotheses)
* Define key terms and concepts, e.g., test anxiety, mature vs traditional students. This can be at the start or somewhere more better positioned to avoid making that first paragraph a list of definitions. Remember citations though.
*	Current study paragraph at the end of introduction can introduce or reiterate the research question if not mentioned before, and usually restates the justification, and ends on hypothesis.
* You must remember to state the hypothesis.
*	Remember the whole point of the introduction is to build to the justification of the (Research Question) RQ and a strong clear hypothesis so try to maintain logical flow by ending on hypothesis after showing all the relevant information first.
* Avoid using bullet points for your hypothesis and, in fact, avoid using bullet points anywhere in the report.
* Avoid numbering hypotheses as H1, H2, etc if you have more than one. It is ok to talk about them though in the terms of the first hypothesis and the second hypothesis.
...
* Does not start on a new page after the end of the Introduction. It starts just after the Introduction.
...
* Materials:
  ...
  * Example questions from the questionnaire are usually shown in quotation marks as it is indicating a specific question. Subscales from the questionnaire and the questionnaire name itself are not usually shown in quotations.
  * There is an example of a write-up of a questionnaire in the slides from the presentation by Phil on writing Methods sections.
* Procedure:
  * Upon reading your procedure a fellow researcher should be able to copy exactly what you did. This doesn't necessarily mean in terms of consents and debriefs, but those can be mentioned briefly ("After completing ...." and "participants were debriefed and thanked for participation"), but really means what a participant saw and/or die when taking part in your study.
  ...
  * There is an example of a write-up of a Procedure in slides of the presentation given by Phil on writing a Methods section.
* Design and Data Analysis
  * This section gives a brief overview of the design of your study and how you are planning on analysing the data.
  * Here you should be looking to state the design (correlational, between-subjects, etc) of your study.
  * You should state the variables for the design as well - remembering to determine for the test which is the dependent variable and which is the independent variable if you are running a t-test. If it is a correlation then they are both just stated as variables.
  ...
  * In terms of data analysis you can state intent to use Pearson or Welch's between-subjects t-test here. You can also present assumption checks, outlier checks, and additional exclusion criteria here or present them in the Results section if more fitting.
...
* When you have more than one analysis it is best to do one whole analysis and then the next and not mixing the two together. The vast majority will only have one analysis however so expect the Results section to be short and to the point.
*	No need for subheadings in a Results sectino but can be used if it would improve the clarity of the writing. For example, "Study 1" and "Study 2" would be fine. We would avoid subheadings of "descriptives", "inferentials" etc. 
...
* All the parts of the results can be written in one or two paragraphs. This looks better than 5 or 6 small one sentence paragraphs.
* When stating the hypothesis, make sure to match it to the one in the introduction. It should not have changed when writing the Methods.
...
* Just to clarify, it is not wrong to show all the histograms and assumption checks but it is not advised because a) at one level you have to trust the researcher and their writing, and b) each figure requires a figure legend and discussion taking up a lot of space and words and will likely be missing stuff from the Introduction or Discussion.
...
* Each analysis usually has one figure. As you are running a t-test or a correlation you would likely have one figure in the report.
*	Put your key figure in the Results section and not in the appendices.
* Just like with tables, refer to your figure in the text (see Figure 1) - interpret and evaluate it in the text, "It would appear that..."
...
* Remember to give a title to tables and figures and to include any notes/legends for the figure. These count as words so keep them short but informative. What does the figure show? What do the dots represent? Try to also think about the figure if it wasn't shown in color - it can help to state something like, "mature students (blue, left) and traditional students (green, right)."  It can also help to indicate on correlation plots or boxplots that show individual data points, what individual data point represents. "Each point shows an individual participant." * Tables and figures count in the word count. You do need at least one figure in the Results section; tables can be used if you like.
...
*	Do not state p-value in scientific format, i.e. p < 6e-12.  That would be stated as simply p < .001
...
* Try to avoid putting a 0 before the decimal place in p-values. It should be p = .012 and not p = 0.012 for example.
*	Give appropriate effect sizes (e.g. cohen's d).
...
* Finish an individual analysis by interpreting results in context of hypothesis (direction and size of difference)
...
* The Discussion should aim to roughly follow the similar idea of the Narrow to Broad structure – summary of results > relate your results to literature and theory mentioned in introduction > possible limitations and future directions > conclusions
...
*	No one way to write a Discussion but basic it would be a summary of findings, relation to papers, relation to theory, caveats and future studies, conclusion.
...
* Create a link to introduction by discussing the findings in relation to some of the findings mentioned in the Introduction. You can of course bring in new papers and research as well and expand the ideas but would be better to also link to those in the introduction and state why these are no longer relevant if that is the case.
  * This is where having some specific relevant details in the Introduction can help with the Discussion. If the results differ, or even if they align, thinking about the nuances of the different studies can help give lots to discuss.
...
*	Don't put the title of the journal article in the main body of the report - for example, in the Introduction, "In the paper, Hocus Pocus got no focus on the Locus of Control by BeetleJuice and colleagues (2010)" - this just wastes words. All you need is the citation. The reference section is for all the other information if someone wants to look up the work.
